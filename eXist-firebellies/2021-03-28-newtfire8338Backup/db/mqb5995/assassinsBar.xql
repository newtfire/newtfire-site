...
return 
    <g>
        <text x="50" y="-550" style="font-size:1.3em;">Color Key:</text>
        <circle cx="80" cy="-480" r="60" fill="blue"/>
        <text x="50" y="-480" fill="white" style="font-size:1.5em;">Action</text>
        <circle cx="210" cy="-480" r="60" fill="pink"/>
        <text x="170" y="-480" fill="black" style="font-size:1.5em;">Speakers</text>
        
        <circle cx="{$pos * $spacer}" cy="-{$countActions * $barElongate + 35}" r="15" stroke="black" fill="blue"/>
        <text x="{$pos * $spacer - 8}" y="-{$countActions * $barElongate + 30}" fill="white">{$countActions}</text>
      <line x1="{$pos * $spacer}" y1="-{$countActions * $barElongate}" x2="{$pos * $spacer}" y2="0" stroke="blue" stroke-width="30"/>
      <text x="{$pos * $spacer + 15}" y="20" style="writing-mode: tb; glyph-orientation-vertical:0">{$sectionTitle}</text>
      <circle cx="{$pos * $spacer + $barSpace}" cy="-{$countSpeakers * $barElongate + 30}" r="15" stroke="black" fill="pink"/>
        <text x="{$pos * $spacer + $barSpace - 8}" y="-{$countSpeakers * $barElongate + 25}" fill="black">{$countSpeakers}</text>
      <line x1="{$pos * $spacer + $barSpace}" y1="-{$countSpeakers * $barElongate}" x2="{$pos * $spacer + $barSpace}" y2="0" stroke="pink" stroke-width="30"/>
    </g>    
...
let $filename := "assassinsBarGraph.svg"
