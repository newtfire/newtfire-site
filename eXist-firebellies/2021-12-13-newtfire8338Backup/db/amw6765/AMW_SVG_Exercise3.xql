...
{ for $e at $pos in $elections
    ...
    let $rElectoral := $candidates/@party["Republican"]/@electoral_votes ! xsd:integer (.)
    let $rPopular := $candidates/@party["Republican"]/@popular_votes ! xsd:integer (.)
    let $dElectoral := $candidates/@party["Democrat"]/@electoral_votes ! xsd:integer (.)
    let $dPopular := $candidates/@party["Democrat"]/@popular_votes ! xsd:integer (.)
    let $pPopular := if ($candidates/[@party="Progressive"])
                            than $e/candidate[@party="Progressive"]/@popular_votes ! xsd:integer (.)
                            else "0"
    let $pElectoral := if ($candidates/[@party ="Progressive"])
                            than $e/candidate[@party="Progressive"]/@electoral_votes ! xsd:integer (.)
                            else "0"
    ...
    <g id="year-{$year}">
        ...
        <circle cx="$year" cy="math:sqrt($party r="")/>
