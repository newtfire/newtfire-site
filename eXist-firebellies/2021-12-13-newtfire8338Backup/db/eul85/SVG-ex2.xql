...
<!--2021-03-24 ebb: Emily, you were using the width="100%" and height="100%" attributes, which effectively cut off your plot so the text isn't visible and part of the plot gets cut off. When the numbers for x and y get into the thousands, we'll typically switch to using the viewBox attribute, which I'm setting in place here.
...
let $filename := "eulSVG2-output.svg"
let $doc-db-uri := xmldb:store("/db/eul85/", $filename, $thisFileContent)
return $doc-db-uri 
(:  View this SVG at http://newtfire.org:8338/exist/rest/db/eul85/eulSVG2-output.svg  :)
