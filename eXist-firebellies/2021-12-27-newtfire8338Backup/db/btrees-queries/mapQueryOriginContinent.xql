...
let $entryOrigins := $entry/treeType ! normalize-space() => distinct-values()
for $o in $entryOrigins

return 
    <Folder>
        <name>{$o}</name>
    { let $entriesByOrigin := $entry[treeType ! normalize-space() = $o]
    for $e in $entriesByOrigin
    let $t := $e/treeType ! normalize-space()
