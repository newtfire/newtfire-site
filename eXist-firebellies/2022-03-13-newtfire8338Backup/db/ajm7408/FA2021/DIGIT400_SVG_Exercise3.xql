...
{ for $e at $pos in $elections
    ...
    let $repubEV := $candidates[@party="Republican"]/@electoral_votes ! xs:integer(.)
    let $repubPV := $candidates[@party="Republican"]/@popular_percentage ! xs:decimal(.)
    let $demEV := $candidates[@party="Democrat"]/@electoral_votes ! xs:integer(.)
    let $demPV := $candidates[@party="Democrat"]/@popular_percentage ! xs:decimal(.)
    let $progEV := if ($candidates[@party="Progressive"])
        then $candidates[@party="Progressive"]/@electoral_votes ! xs:integer(.)
        else "0"
    ...
    <g id="year-{$year}">
        ...
        <circle cx="{}" cy="5" r="50" fill="black" opacity=".25"/>    
