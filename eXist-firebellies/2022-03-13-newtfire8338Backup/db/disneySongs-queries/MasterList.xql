...
return <person xml:id="{string-join($ref, ',')}">{$va}</person>}</listVoiceActor>
    <listRoles>
        {let $voiceActor := $collection//voiceActor ! normalize-space () => distinct-values() => sort()
for $va in $voiceActor
let $role := $collection//voiceActor[. ! normalize-space() =$va ]/@role ! normalize-space () ! tokenize(.,"#")[last()] => distinct-values()

return <role xml:id="{string-join($role, ',')}">{$va}</role>}</listRoles>

</xml>
;
let $filename := "listofRef.xml"
