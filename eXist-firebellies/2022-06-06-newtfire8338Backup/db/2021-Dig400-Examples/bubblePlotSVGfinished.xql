...
<!-- dashed-line hashes for electorals -->
{for $i in (0 to 5)
return
  <g>    
...
<!-- Now, loop throug the election data, and for each election year, loop through the candidates. Plot circles at the appropriate y positions, with circle areas calculated based on percentages of popular votes. Give each party candidate a distinct color. -->
{ for $e at $pos in $elections
(: $pos is a position number in the for loop. Use it to $pos values across the x axis.:)
    ...
    <g id="year-{$year}">
       ...
       <circle class="repub" cx="{$pos * $xSpacer}" cy="{-$repubEV}" r="{math:sqrt(math:pi() * $repubPV * $repubPV) div math:pi()}" fill="pink"/>
       <text x="{$pos * $xSpacer + 20}" y="{-$repubEV - 10}">{$repubPV}%</text>
       
       <circle class="dem" cx="{$pos * $xSpacer}" cy="{-$demEV}" r="{math:sqrt(math:pi() * $demPV * $demPV) div math:pi()}" fill="deepskyblue"/>
       <text x="{$pos * $xSpacer + 20}" y="{-$demEV - 10}">{$demPV}%</text>
       
        <circle class="prog" cx="{$pos * $xSpacer}" cy="{-$progEV}" r="{math:sqrt(math:pi() * $progPV * $progPV) div math:pi() * $circleSizer}" fill="seagreen"/>
       <text x="{$pos * $xSpacer + 20}" y="{-$progEV }">{$progPVtext}</text>
       <circle class="soc" cx="{$pos * $xSpacer}" cy="{-$socEV}" r="{math:sqrt(math:pi() * $socPV * $socPV) div math:pi() * $circleSizer}" fill="orange"/>
