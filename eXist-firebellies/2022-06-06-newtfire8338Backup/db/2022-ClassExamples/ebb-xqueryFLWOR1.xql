...
(:  : This variable returns the count of Falstaff's speeches in the ENTIRE collection. let $countFals := $falstaffs => count() :)
let $falPlays := $shakes//TEI[descendant::sp[contains(@who, "Falstaff")]]
for $f in $falPlays
   let $fTitle := $f//titleStmt/title ! string()
