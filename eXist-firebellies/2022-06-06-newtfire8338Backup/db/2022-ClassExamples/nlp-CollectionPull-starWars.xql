...
let $filename := $s/base-uri() ! tokenize(., '/')[last()] ! substring-before(., 'xml') ! concat(., 'txt')
let $thisFileContent := 
...
let $doc-db-uri := xmldb:store("/db/2022-ClassExamples", $filename, $thisFileContent, "text/plain")
return $doc-db-uri 
