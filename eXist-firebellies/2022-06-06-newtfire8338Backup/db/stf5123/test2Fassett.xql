...
declare variable $spCounts := 
    for $s in $segments
    let $sections := $s//Q{}sp 
    let $countSections := $sections => count()=> distinct-values()
    order by $countSections descending
...
<html> 
    ...
    <body>
    <h1>Speeech count, section title and number of speakers in Assassins Creed Odyssey. </h1>
    
    <table>
        <tr><th>Speech Count</th><th>Section Title</th>  <th>Number of Speakers</th></tr>
{
...
return 
    <tr>
        <td>{$spCounts}</td> <td>{$sectionTitle}</td> <td>{$countSpeakers}</td>
        </tr>
}
 </table>
</body>
