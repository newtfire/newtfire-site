...
(: ebb: You gave a mostly good explanation of the $sortOrder variable, 
 : but seem not to have understood why we need it for your output. 
 : So, notice when you plot your output, it is not actually coming out in chronological order! 
 : I replaced the $title with the $date variable for the labels for your bars to demonstrate.
 : order by won't work inside the for loop below, because you are not returning the entire sequence of 
 : of drafts, just one at a time.  We need the $sortOrder variable above to store the entire 
 : sequence of drafts in chronological order *before* we loop over them one by one. So the idea here
 : was to write the loop below as 
...
(:return concat($pos,", Date Created: ", $date,", Title of Draft: ", $title,", Speech Count: ", $sp,", Distinct Speakers: ", $spk)  Question 8 :)
  return
      <g>
          ...
          <text x="{$pos * $xSpacer}" y="20" fill="#626262" font-size="15" transform="rotate(45 {$pos * $xSpacer}, 20)">{$date}</text>
      </g>
