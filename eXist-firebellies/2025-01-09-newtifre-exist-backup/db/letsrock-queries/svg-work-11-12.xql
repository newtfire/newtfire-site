...
<svg  width="2000" height="3000">
   ...
   <line x1="0" y1 ="0" x2="0" y2="-3300" stroke="blue" stroke-width="3"></line>
    ...
    return
        ...
        <!-- ebb: Consider outputting this as a circle like we did for the bubble chart, and plot up above the top 
       ...
       <rect x="{$spacer * $pos}" y="0" width="40" height="{$sumSL}" fill="{$colors[position() = $pos]}" />
        ...
        <text x="{$spacer * $pos + 40}" y="{-($count div 10 )}" >{$ds}</text> (:The plus 40 allows the text to perfectly begin right after the bar appears so I could work on rotating them:)
