...
return concat($pos, "&#x9;"(:tab character:), $w (:source node:), "&#x9;"(:tab character:), $wordType (:source attribute:), "&#x9;"(:tab character:), "appears this many times in" (:edge connector or shared interaction:), "&#x9;", $wCount (:edge attribute:), "&#x9;", $mTitle (:target node:), "&#x9;"(:tab character:), "poemTitle" (:target attribute:)), "&#10;" ); 

let $filename := "piperNetWordtoPoem.tsv"
let $doc-db-uri := xmldb:store("/db/piperpoetry/", $filename, $poetrymaster, "text/plain")
return $doc-db-uri
(: View this text file at http://newtfire.org:8338/exist/rest/db/piperpoetry/piperNetWordtoPoem.tsv  :)