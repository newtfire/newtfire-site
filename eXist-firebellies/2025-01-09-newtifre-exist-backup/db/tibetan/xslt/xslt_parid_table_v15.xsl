...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xpath-default-namespace="http://www.bcrdb.org/ns/1.0">
    ...
    <xsl:variable name="tablewidth" select="370 * $numWitness"/>
    <xsl:template match="/">
        ...
        <html>
            ...
            <body>
                ...
                <h3>
                    <center>ver. 3.0 (beta 15b)</center>
                </h3>
                <h2>
        ...
        <xsl:choose>
            <xsl:when test="@col = '1'">
                ...
                <td width="{$cellwidth}%" bgcolor="#00c3fb">
                    <!-- xsl:value-of select="concat('position = ', $position, ' col = ', @col, ' last = ', $last, ' ', 'cell width', $cellwidth)"/>
                ...
                <xsl:if test="(($position = count(parent::list/div)) and ($position &lt; $numWitness))">
                    <td width="{$cellwidth}%" bgcolor="#ff00ff">
                        <xsl:variable name="status" select="$position"/>
                        <xsl:value-of select="' '"/>
                    </td>
                </xsl:if>
                <xsl:if test="(($position = count(parent::list/div)) and (($position + 1) &lt; $numWitness))">
                    <td width="{$cellwidth}%" bgcolor="#cc6699">
                        <xsl:value-of select="' '"/>
                    </td>
                </xsl:if>
                <xsl:if test="(($position = count(parent::list/div)) and (($position + 2) &lt; $numWitness))">
                    <td width="{$cellwidth}%" bgcolor="#00aa22">
                        <xsl:value-of select="' '"/>
                    </td>
                </xsl:if>
                <xsl:if test="(($position = count(parent::list/div)) and (($position + 3) &lt; $numWitness))">
